...
void vert (inout appdata_full v, out Input o)
{
	...
	o.MainTexUV = TRANSFORM_TEX (v.texcoord, _MainTex);
		...
		//Rotate	
		#if !defined(V_FR_R_OFF)
			float theta = _Time.y * _RotSpeed;
			theta *= fragmentArea * min(fragmentFactor, 1);		
		
			#if defined(V_FR_RO_ON)
				v.vertex.xyz = V_FR_RotateOrigin(v.vertex, buf_Normal[index], theta);
			#endif

			#if defined(V_FR_RC_ON)				
